...
function WebPlaceShow() {
  ...
  const fetchDataPlace = async () => {
    ...
    await Api.get(`/api/web/places/${slug}`).then((response) => {
      ...
      document.title = `${response.data.data.title} - Si Asri`
    })
  }
              ...
              <div className='card border-0 rounded shadow-sm'>
                <div className='card-body'>
                  ...
                  <div className='d-grid gap-2'>
                    <Link
                      to={`/lokasi/${place.slug}/direction?longitude=${place.longitude}&latitude=${place.latitude}`}
                      className='float-end btn btn-success btn-block btn-md mt-3'
                    >
                      <i className='fa fa-location-arrow'></i> ANTAR OBAT KE
                      {'\t'}
                      {place.title}
                    </Link>
                  </div>
